...
const PrivacyPolicy: React.FC<PrivacyPolicyProps> = ({ language, onBack }) => {
  const content = {
    en: {
      ...
      sections: [
        ...
        },
        {
          ...
          p: 'When you chat with Nur AI, your messages are processed by Google Gemini API. These interactions are subject to Google\'s privacy terms. We do not use your chat history for advertising.'
        },
        {
          h: '3. Location Services',
          p: 'We request location access solely to calculate accurate prayer times. Your coordinates are used locally or sent to open-source geocoding APIs (like Nominatim) to display your city name. We do not track your movement history.'
        },
        {
    ...
    bn: {
      ...
      lastUpdated: 'সর্বশেষ আপডেট: অক্টোবর ২০২৩',
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 pb-20 max-w-2xl mx-auto">
      ...
      <div className="glass p-8 rounded-[2.5rem] border-white/5 space-y-8 leading-relaxed text-slate-300">
        <p className="font-medium text-[var(--brand-olive)]">{content.intro}</p>
        
        {content.sections.map((s, i) => (
          <section key={i} className="space-y-3">
            <h3 className="text-white font-bold">{s.h}</h3>
            <p className="text-sm opacity-80">{s.p}</p>
          </section>
        ))}

        <div className="pt-8 border-t border-white/5 text-[10px] text-slate-500 text-center uppercase tracking-widest">
